...
;---------------------------------------------------------------------------------------------------------
;Definition Protocols and Records
...
(defrecord RasterSearchContainer
  ...
  (demand-set  [this given-raster]
    (->demandSet (get-demand-units {:raster given-raster} demand-quartiles)))
...
(defrecord PointSearchContainer
  ...
  (demand-set [this]
    (-> demandSet (get-demand-units {:sources-data sources-data} demand-quartiles)))
  (selected-locations [this] selected-locations)
  (coverage [this] coverage-fn)
  (radius-coverage [this]
    (or bound
...
(defn next-center
  ([demand center radius]
   (next-center demand center radius (/ radius 1000)))
...
(defn find-centroid-from-demand-unit
  ...
  (let [demand      (remove-first
    ...
    (loop [sum      0
      ...
      (if (<= (- avg-max sum) 0)
        center
        (let [next-center (next-center demand center radius)
...
(defn- create-raster-with-new-data
...
(defn update-searching-raster
  ...
  (let [raster (create-raster-with-new-data (raster/read-raster search-path) [lon lat])]
...
(defn raster-update-demand-set
  ...
  (let [searching-raster (raster/read-raster (:search-path search-container))
        ...
        final-center (find-centroid-from-demand-unit center search-container)
